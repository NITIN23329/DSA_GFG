...
*/
    ...
    // note that the average can not be 0 ,so we do not need any initialization by -1.
        public double memoization(int[] A, int K) {
            int n = A.length;
            int[] sum = new int[n+1];
            for(int i=0;i<n;i++)
                sum[i+1] = sum[i] + A[i];
            double[][][] dp = new double[n][n][K+1];
            return memoHelper(A,K,0,0,sum,dp);
        }
        private double memoHelper(int[] arr, int k, int l, int r, int[] sum, double[][][] dp){
            if(r==arr.length)return 0;
            if(k==1)return (sum[arr.length]-sum[l])/(arr.length-l+0d);
            if(dp[l][r][k]!=0)return dp[l][r][k];
            double c1 = (sum[r+1]-sum[l])/(r-l+1d) + memoHelper(arr,k-1,r+1,r+1,sum,dp);
            double c2 = memoHelper(arr,k,l,r+1,sum,dp);
