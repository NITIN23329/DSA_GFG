...
// So finding max Value in range l to r takes O(1) time.
        ...
        public int memoization(int[] arr, int k) {
            int n = arr.length;
            int[][] max = new int[n][n];
            for(int l=0;l<n;l++){
                max[l][l] = arr[l];
            ...
            for(int i=0;i<n;i++)
                Arrays.fill(dp[i],-1);
            return memoHelper(arr,0,0,k,dp,max);
        }
        private int memoHelper(int [] arr, int l, int r, int k, int[][] dp, int[][] max){
            if(r==arr.length)return 0;
            if(dp[l][r]!=-1)return dp[l][r];
            if(r-l+1<=k){
                int maxVal = max[l][r];
                return dp[l][r] = Math.max((r-l+1)*maxVal + memoHelper(arr,r+1,r+1,k,dp,max), memoHelper(arr,l,r+1,k,dp,max));
            }
            return  dp[l][r] = memoHelper(arr,r+1,r+1,k,dp,max);
        }
...
*/
        public int bottomUp(int[] arr, int k) {
            int n = arr.length;
            int[] dp = new int[n];
            for(int i=0;i<n;i++){
                int max = 0;
