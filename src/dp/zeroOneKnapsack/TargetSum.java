...
public class TargetSum {
    ...
    private int recursiveHelpr(int i,int curr,int sum,int[] arr){
        ...
        return recursiveHelpr(i+1,curr+arr[i],sum,arr) +
                recursiveHelpr(i+1,curr-arr[i],sum,arr);
    }
    ...
    private int memoHelpr(int i,int curr,int sum,int[] arr,int [][] dp){
        ...
        return dp[i][curr+1000] = memoHelpr(i+1,curr+arr[i],sum,arr,dp) +
                memoHelpr(i+1,curr-arr[i],sum,arr,dp);
    }
